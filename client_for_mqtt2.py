...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.pushButton_ghost_w.clicked.connect(
            lambda: self.showMinimized())
        self.ui.lineEdit_for_writetext2.setPlaceholderText(
            'Ввести текст для mqtt/text/chat')
    ...
    def public(self, client):
        ...
        if subtop == 'device/memorystatus/harddrive/c':
            self.ui.textEdit_for_view2.insertPlainText(
                '[' + cur_time + '] ' + '<' + str(self.hdd.free / (2**30)) + '> ' + '\n')

        if subtop == 'device/work/cpu':
            self.ui.textEdit_for_view2.insertPlainText(
                '[' + cur_time + '] ' + '<' + str(psutil.cpu_percent(interval=1)) + '> ' + '%' + '\n')

        if subtop == 'mqtt/text/chat':
            self.ui.textEdit_for_view2.insertPlainText(
                '[' + cur_time + '] ' + '<mqtt/text/chat>  ' + text + '\n')
        ...
        if subtop == 'music/track1/start':
            self.ui.textEdit_for_view2.insertPlainText(
                '[' + cur_time + '] ' + '<команда старт для Track1> ' + '\n')

        if subtop == 'music/track1/stop':
            self.ui.textEdit_for_view2.insertPlainText(
                '[' + cur_time + '] ' + '<команда стоп для Track1> ' + '\n')

        if subtop == 'android/get_ascii':
            self.ui.textEdit_for_view2.insertPlainText(
                '[' + cur_time + '] ' + '<запрос на ascii> ' + '\n')

        if subtop == 'android/get_ip':
            self.ui.textEdit_for_view2.insertPlainText(
                '[' + cur_time + '] ' + '<запрос на ip> ' + '\n')

        if subtop == 'android/tts':
            self.ui.textEdit_for_view2.insertPlainText(
                '[' + cur_time + '] ' + '<запрос на речь> ' + '\n')
